...
for i = 1:6
    ...
    if (i==1) || (i==2) || (i==5) % only plots that have the contours are plotted here
        ...
        colorbar(figaxesTCJleft,'Location','East','Direction', 'reverse','Position',[.93 .11 .036 .815]); caxis([2 21]); % the scale is fixed.
        ...
        % colorbar is linked to the appropriate handle and it is positioned properly with limits to include whole of TCJleft and TCJright values
        colorbar(figaxesTCJright,'Location','East','Direction', 'reverse','Position',[.93 .11 .036 .815]); caxis([2 21]); % the scale is fixed.
        ...
        % colorbar is linked to the appropriate handle and it is positioned properly with limits to include whole of TCJleft and TCJright values
        colorbar(fig3axesTCJleft,'Location','East','Direction', 'reverse','Position',[.93 .11 .036 .815]); caxis([2 21]); % the scale is fixed.
        ...
        % colorbar is linked to the appropriate handle and it is positioned properly with limits to include whole of TCJleft and TCJright values
        colorbar(fig3axesTCJright,'Location','East','Direction', 'reverse','Position',[.93 .11 .036 .815]); caxis([2 21]); % the scale is fixed.
...
% zero or a smaller value (in this case we take 0.5 as smaller value), then it is replaced with "one". In this case, instead of infinity or a very high value, the value of numerator will be the value of ratio. 
% On the other hand, if the left (numerator) is zero, we leave it as zero so that the value of ratio becomes zero.
area_under_the_curve_right(area_under_the_curve_right < 0.5) = 1; % changing all values below "0.5" to "one"
